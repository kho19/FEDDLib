...
NonLinLaplace<SC, LO, GO, NO>::NonLinLaplace(const DomainConstPtr_Type &domain, std::string FEType,
                                             ParameterListPtr_Type parameterList)
    : NonLinearProblem<SC, LO, GO, NO>(parameterList, domain->getComm()), u_rep_() {
...
template <class SC, class LO, class GO, class NO> void NonLinLaplace<SC, LO, GO, NO>::info() {
    this->infoProblem();
 ...
 */
template <class SC, class LO, class GO, class NO> void NonLinLaplace<SC, LO, GO, NO>::assemble(std::string type) const {
    if (type == "") {
        if (this->verbose_) {
 ...
 */
template <class SC, class LO, class GO, class NO> void NonLinLaplace<SC, LO, GO, NO>::initAssemble() const {
...
template <class SC, class LO, class GO, class NO>
void NonLinLaplace<SC, LO, GO, NO>::reAssembleExtrapolation(BlockMultiVectorPtrArray_Type previousSolutions) {

    TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "Only Newton/NOX implemented for nonlinear Laplace!");
}
 ...
 */
template <class SC, class LO, class GO, class NO>
void NonLinLaplace<SC, LO, GO, NO>::evalModelImpl(const Thyra::ModelEvaluatorBase::InArgs<SC> &inArgs,
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(this->solution_->getBlock(0)->getMap()->getUnderlyingLib() != "Tpetra",
                               std::runtime_error,
    ...
    if (fill_f || fill_W || fill_W_prec) {
        ...
        if (fill_W) {
            ...
            Xpetra::CrsMatrixWrap<SC, LO, GO, NO> &crsOp =
                dynamic_cast<Xpetra::CrsMatrixWrap<SC, LO, GO, NO> &>(*W_systemXpetraNonConst);
            Xpetra::TpetraCrsMatrix<SC, LO, GO, NO> &xTpetraMat =
                dynamic_cast<Xpetra::TpetraCrsMatrix<SC, LO, GO, NO> &>(*crsOp.getCrsMatrix());
            ...
            for (auto i = 0; i < tpetraMatXpetra->getMap()->getLocalNumElements(); i++) {
                typename Tpetra::CrsMatrix<SC, LO, GO,
                                           NO>::local_inds_host_view_type indices; // ArrayView< const LO > indices
        ...
        if (fill_W_prec) {
            ...
            std::string levelCombination = this->parameterList_->sublist("ThyraPreconditioner")
 ...
 */
template <class SC, class LO, class GO, class NO>
Teuchos::RCP<Thyra::LinearOpBase<SC>> NonLinLaplace<SC, LO, GO, NO>::create_W_op() const {
 ...
 */
template <class SC, class LO, class GO, class NO>
Teuchos::RCP<Thyra::PreconditionerBase<SC>> NonLinLaplace<SC, LO, GO, NO>::create_W_prec() const {
