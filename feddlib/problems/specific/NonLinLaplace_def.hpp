...
void NonLinLaplace<SC, LO, GO, NO>::reAssemble(std::string type) const {
  ...
  if (type == "Rhs") {
    ...
    this->feFactory_->assemblyNonlinearLaplace(
        this->dim_, this->getDomain(0)->getFEType(), 2, u_rep_, this->system_,
        this->residualVec_, this->parameterList_, "Rhs");

  } else if (type == "Newton") {

    this->feFactory_->assemblyNonlinearLaplace(
        this->dim_, this->getDomain(0)->getFEType(), 2, u_rep_, this->system_,
        this->residualVec_, this->parameterList_, "Jacobian");
  }
  if (this->verbose_)
    std::cout << "done -- " << std::endl;
}
...
void NonLinLaplace<SC, LO, GO, NO>::evalModelImpl(
  ...
  if (fill_f || fill_W || fill_W_prec) {
    ...
    if (fill_W) {
      ...
      for (auto i = 0; i < tpetraMatXpetra->getMap()->getLocalNumElements();
           i++) {
        typename Tpetra::CrsMatrix<SC, LO, GO, NO>::local_inds_host_view_type
            indices; // ArrayView< const LO > indices
        typename Tpetra::CrsMatrix<SC, LO, GO, NO>::values_host_view_type
            values;
    ...
    if (fill_W_prec) {
      ...
      // ch 26.04.19: After each setup of the preconditioner we check if we use
      // a two-level precondtioner with multiplicative combination between the
      // levels. If this is the case, we need to pre apply the coarse level to
      // the residual(f_out).
      ...
      if (!levelCombination.compare("Multiplicative")) {
        TEUCHOS_TEST_FOR_EXCEPTION(
            true, std::logic_error,
            "Multiplicative Level Combination is not supported for NOX. In "
            "general we need to pre-apply the coarse problem. This must be "
            "implemented here.");
      }
    }
  }
