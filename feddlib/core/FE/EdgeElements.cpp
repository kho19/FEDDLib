...
EdgeElements::EdgeElements()
    : Elements(), elementsOfEdgeGlobal_(0), elementsOfEdgeLocal_(0), edgesOfElements_(0), midPointsLocalID_(0){
...
EdgeElements::EdgeElements(EdgeElements &elements)
    : Elements(*(Teuchos::rcp_dynamic_cast<Elements_Type>(Teuchos::rcpFromRef(elements)))),
      elementsOfEdgeGlobal_(elements.elementsOfEdgeGlobal_), elementsOfEdgeLocal_(elements.elementsOfEdgeLocal_),
      edgesOfElements_(0), midPointsLocalID_(0){

                           };

void EdgeElements::addEdge(FiniteElement &fe, GO globalID) {
    this->addElement(fe);
    elementsOfEdgeGlobal_.push_back(
        vec_GO_Type(1, globalID)); // Global ID refers to the element, to which we add the edge
};
void EdgeElements::setElementsEdges(vec2D_GO_Type &elementsOfEdges) {
    ...
    // Here we want to set all elements for an edge. We have the information in elementsOfEdges, which holds the
    // information from unique to redundant edges. and elementsOfEdgeGlobal_, which holds the global element ID.
...
void EdgeElements::partitionEdges(MapConstPtr_Type elementMap, MapConstPtr_Type nodeMapRepeated) {
    ...
    // Here it is assumed that elementsOfEdgeGlobal_ is still the list for redundant edges. The correct partitioned and
    // combine list is only set here.
    // TODO We might want to shift the setup of combined element information to the function sortUniqueAndSetGlobalIDs()
    // in the future
...
void EdgeElements::sortUniqueAndSetGlobalIDs(vec2D_GO_Type &combinedElements) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(elements_.is_null(), std::runtime_error,
                               "Elements not initialized! sortUniqueAndSetGlobalIDs( ) not possible.");
...
void EdgeElements::makeUniqueWithCombines(FE_vec_ptr_Type &elements, vec2D_GO_Type &combinedElements,
                                          vec2D_GO_Type &globaIDs){
    // We assume that each inner vector of globalIDs has only one element which means that globalIDs can be represented
    // by a vec_GO_Type. However, we want to use vec2D_GO_Type here because we would have to copy the values otherwise
    // in the method sortUniqueAndSetGlobalIDs(), where this method is used
...
vec2D_GO_Type EdgeElements::sort_from_ref(vec2D_GO_Type const &in, std::vector<int> const &reference) {
...
const vec_LO_Type &EdgeElements::getElementsOfEdge(int i) {
    TEUCHOS_TEST_FOR_EXCEPTION(elementsOfEdgeLocal_.size() - 1 < i, std::logic_error,
                               "No local elements for this edge.");

    return elementsOfEdgeLocal_[i];
};

const vec_GO_Type &EdgeElements::getElementsOfEdgeGlobal(int i) {
    TEUCHOS_TEST_FOR_EXCEPTION(elementsOfEdgeGlobal_.size() - 1 < i, std::logic_error,
                               "No global elements for this edge.");
    return elementsOfEdgeGlobal_[i];
};

// returns the edges of Element i
const vec_int_Type EdgeElements::getEdgesOfElement(int i) {
    TEUCHOS_TEST_FOR_EXCEPTION(edgesOfElements_.size() - 1 < i, std::logic_error,
                               "No edges for this Element. Either your requested index is out of bounds or the edges "
                               "of Elements wasn't set up yet.");
...
int EdgeElements::getMidpoint(LO edgeId) {
    TEUCHOS_TEST_FOR_EXCEPTION(midPointsLocalID_.size() - 1 < edgeId, std::logic_error,
                               "No midpoint index was set for the requested edge. Either you are out of bounds or "
                               "haven't initialized the mipoint yet.");
    if (midPointsLocalID_.size() < 1)
        return -1;
    else
...
void EdgeElements::setElementsOfEdgeLocalEntry(int index, int entry) { elementsOfEdgeLocal_[index].push_back(entry); }

void EdgeElements::setElementsOfEdgeGlobalEntry(int index, int entry) { elementsOfEdgeGlobal_[index].push_back(entry); }

// Parallel Functions for building edgelists, elementsOfEdgeGlobal and elementsOfEdgesLocal
// The difference is, that we dont build the elementsVec and use the elementMap to set the right global IDs to
// elementsOfEdgeGlobal
void EdgeElements::sortUniqueAndSetGlobalIDsParallel(MapConstPtr_Type elementMap, vec2D_GO_Type &combinedElements) {
    ...
    // We now have elemenetsOfEdgeGlobal_ but without the entries for elements on other procs. As of now
    // elementsOfEdgeLocal_ is the same just missing the -1 entries, that we add later
...
// IMPORTANT: this alone does not update elementsOfEdgeLocal and elementsOfEdgeGlobal completetly, it only sets the
// vectors up with information that is available on the own processor In order to completly set those vectors up, one
// has to communicate across the interface. This is done in meshUnstructuredRefinement with
...
void EdgeElements::setUpElementsOfEdge(MapConstPtr_Type elementMap, MapConstPtr_Type edgeMap) {
    ...
    // Here it is assumed that elementsOfEdgeGlobal_ is still the list for redundant edges. The correct partitioned and
    // combine list is only set here. We might want to make shift the setup of combined element information to the
    // function sortUniqueAndSetGlobalIDs() in the future
