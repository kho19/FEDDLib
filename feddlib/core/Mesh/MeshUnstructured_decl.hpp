...
/*!
        Extension of mesh class. MeshUnstructured represents meshes that are read through .mesh input file. In contrast
   to structured meshes which are meshes we build internally. Features functions to read a mesh and extend it to a P2
   mesh.
*/
...
public:
    ...
    typedef typename Mesh_Type::Graph_Type Graph_Type;
    typedef typename Mesh_Type::GraphPtr_Type GraphPtr_Type;
    typedef typename Mesh_Type::GraphFactory_Type GraphFactory_Type;
    ...
    */
    ...
    /*!
            \brief Helper function for setP2SurfaceElements. Adds the correct nodes to the meshP1 subelements. Based on
       sorted Elements.
    ...
    */
    void setSurfaceP2(FiniteElement &feP2, const FiniteElement &surfFeP1, const vec2D_int_Type &surfacePermutation,
                      int dim);

    /*!
            \brief Helper function for setP2SurfaceElements. Adds the correct nodes to the meshP1 subelements. Based on
       unsorted elements. Different approach than above. Based on edge Midpoints.
            ...
            @param[in] dim Dimension
    */
    void addSurfaceP2Nodes(FiniteElement &feP2, const FiniteElement &surfFeP1, const vec2D_int_Type &surfacePermutation,
                           int dim);
    /*!
            \brief Depending on the sorting of P1 surface nodes we have to adjust the new ordering of P2 edge midpoints
       for surfaces in 3D
    */
    ...
    */
    ...
    /*!
            ...
            @param[in] positions
            @param[in] elementsGlobalOfEdge Global IDs of elements connected to the edge (numbering needs to be
       consistent)
            @param[in] p1ID Local ID of first node of edge
            @param[in] p2ID Local ID of second node of edge
            @param[in] meshP1 The p1 mesh we use for building P2 mesh
    */
    void determinePositionInElementP2(vec_int_Type &positions, vec_GO_Type &elementsGlobalOfEdge, LO p1ID, LO p2ID,
    ...
    */
    ...
    /*!
            \brief Essentially determine flag of an edge. Thus determine P2 Flag for building P2 mesh. Longer version of
       other determineFlagP2 function. Also informs whether flag could be found or not (not always the case in parallel)
            @param[in] p1ID Local ID of first node of edge
            @param[in] p2ID Local ID of second node of edge
