...
MeshStructured<SC, LO, GO, NO>::MeshStructured() : Mesh<SC, LO, GO, NO>(), coorRec(0), length(0), height(0), width(0) {}

template <class SC, class LO, class GO, class NO>
MeshStructured<SC, LO, GO, NO>::MeshStructured(CommConstPtrConst_Type &comm)
    : Mesh<SC, LO, GO, NO>(comm), coorRec(0), length(0), height(0), width(0) {}

template <class SC, class LO, class GO, class NO> MeshStructured<SC, LO, GO, NO>::~MeshStructured() {}
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildMesh2DTPM(std::string FEType, int N, int M, int numProcsCoarseSolve,
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildMesh2DMiniTPM(std::string FEType, int N, int M, int numProcsCoarseSolve,
                                                        std::string underlyingLib) {
...
template <class SC, class LO, class GO, class NO> void MeshStructured<SC, LO, GO, NO>::buildSurfaceLinesSquare() {
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildMesh2D(std::string FEType, int N, int M, int numProcsCoarseSolve,
                                                 std::string underlyingLib) {
    ...
    // Set number of ranks on fine level
    setRankRange(numProcsCoarseSolve);
    ...
    if (FEType == "P0") {
        ...
        // N = num domains in 1D. M = num elements per domain
        // M+1 points per domain. Subtract double counted points
    ...
    // Each square between points is split into two triangles
    ...
    if (FEType == "P1") {
        ...
        // If rank greater than first row, adjust y offset to appropriate row
        if ((rank % (N * N)) >= N) {
        ...
        for (int s = 0; s < M + 1; s++) {
            for (int r = 0; r < M + 1; r++) {
                ...
                // Set to zero if almost zero
                if ((*this->pointsRep_)[counter][0] < 100 * ScalarTraits<SC>::eps() &&
                    (*this->pointsRep_)[counter][0] > -100 * ScalarTraits<SC>::eps()) {
                    (*this->pointsRep_)[counter][0] = 0.0;
                }
                // Repeat for y coord
                (*this->pointsRep_)[counter][1] = s * h + offset_y * H;
                if ((*this->pointsRep_)[counter][1] < 100 * ScalarTraits<SC>::eps() &&
                    (*this->pointsRep_)[counter][1] > -100 * ScalarTraits<SC>::eps()) {
                    (*this->pointsRep_)[counter][1] = 0.0;
                }
                // Determine global index of current point
                pointsRepGlobMapping[counter] =
                    r + s * nmbPoints_oneDir + offset_x * (M) + offset_y * (nmbPoints_oneDir)*M;
                // Set BC flag if close to edge
                if ((*this->pointsRep_)[counter][0] > (coorRec[0] + length - 100 * ScalarTraits<SC>::eps()) ||
                    (*this->pointsRep_)[counter][0] < (coorRec[0] + 100 * ScalarTraits<SC>::eps()) ||
                    (*this->pointsRep_)[counter][1] > (coorRec[1] + height - 100 * ScalarTraits<SC>::eps()) ||
                    (*this->pointsRep_)[counter][1] < (coorRec[1] + 100 * ScalarTraits<SC>::eps())) {
        ...
        // Set the repeated map
        this->mapRepeated_.reset(new Map<LO, GO, NO>(underlyingLib, (GO)-1, pointsRepGlobMapping(), 0, this->comm_));
        // Build unique map from repeated map
        this->mapUnique_ = this->mapRepeated_->buildUniqueMap(numProcsCoarseSolve);
        ...
        for (int s = 0; s < M; s++) {
            for (int r = 0; r < M; r++) {
                ...
                // Calculate centroid of the triangle (centroid formula)
                x_ref = (this->pointsRep_->at(elementsVec->at(counter).at(0)).at(0) +
                         this->pointsRep_->at(elementsVec->at(counter).at(1)).at(0) +
                         this->pointsRep_->at(elementsVec->at(counter).at(2)).at(0)) /
                        3.;
                y_ref = (this->pointsRep_->at(elementsVec->at(counter).at(0)).at(1) +
                         this->pointsRep_->at(elementsVec->at(counter).at(1)).at(1) +
                         this->pointsRep_->at(elementsVec->at(counter).at(2)).at(1)) /
                        3.;
                // Set elementFlag if centroid fulfills requirements
                // What is the elements flag used for? If TPMSquare then set to 1?
                // Used in Elasticity assembly to give different mue to elements
                // In mesh files they are all set to 10
    ...
    // P2 Mesh
        ...
        for (int s = 0; s < 2 * (M + 1) - 1; s++) {
            for (int r = 0; r < 2 * (M + 1) - 1; r++) {
                ...
                pointsRepGlobMapping[counter] = r + s * nmbPoints_oneDir + offset_x * (2 * (M + 1) - 2) +
                                                offset_y * (nmbPoints_oneDir) * (2 * (M + 1) - 2);
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildMesh3D(std::string FEType, int N, int M, int numProcsCoarseSolve,
                                                 std::string underlyingLib) {
    ...
    if (FEType == "P1") {
        ...
        for (int t = 0; t < M + 1; t++) {
            for (int s = 0; s < M + 1; s++) {
                for (int r = 0; r < M + 1; r++) {
                    (*this->pointsRep_)[counter][0] = r * h + offset_x * H;
                    if ((*this->pointsRep_)[counter][0] < eps && (*this->pointsRep_)[counter][0] > -eps)
                        (*this->pointsRep_)[counter][0] = 0.0;

                    (*this->pointsRep_)[counter][1] = s * h + offset_y * H;
                    if ((*this->pointsRep_)[counter][1] < eps && (*this->pointsRep_)[counter][1] > -eps)
                        (*this->pointsRep_)[counter][1] = 0.0;

                    (*this->pointsRep_)[counter][2] = t * h + offset_z * H;
                    if ((*this->pointsRep_)[counter][2] < eps && (*this->pointsRep_)[counter][2] > -eps)
                        (*this->pointsRep_)[counter][2] = 0.0;

                    pointsRepGlobMapping[counter] = r + s * nmbPoints_oneDir + t * nmbPoints_oneDir * nmbPoints_oneDir +
                                                    offset_x * (M) + offset_y * (nmbPoints_oneDir)*M +
                                                    offset_z * (nmbPoints_oneDir) * (nmbPoints_oneDir)*M;

                    if ((*this->pointsRep_)[counter][0] > (coorRec[0] + length - eps) ||
                        (*this->pointsRep_)[counter][0] < (coorRec[0] + eps) ||
                        (*this->pointsRep_)[counter][1] > (coorRec[1] + width - eps) ||
                        (*this->pointsRep_)[counter][1] < (coorRec[1] + eps) ||
                        (*this->pointsRep_)[counter][2] > (coorRec[2] + height - eps) ||
                        (*this->pointsRep_)[counter][2] < (coorRec[2] + eps)) {
        ...
        for (int t = 0; t < 2 * (M + 1) - 1; t++) {
            for (int s = 0; s < 2 * (M + 1) - 1; s++) {
                for (int r = 0; r < 2 * (M + 1) - 1; r++) {
                    ...
                    pointsRepGlobMapping[counter] =
                        r + s * nmbPoints_oneDir + t * nmbPoints_oneDir * nmbPoints_oneDir +
                        offset_x * (2 * (M + 1) - 2) + offset_y * (nmbPoints_oneDir) * (2 * (M + 1) - 2) +
                        offset_z * (nmbPoints_oneDir) * (nmbPoints_oneDir) * (2 * (M + 1) - 2);
        ...
        for (int i=0; i<this->mapUnique_->getNodeNumElements(); i++) {
            ...
            (*this->pointsUni_)[i][1] =
                ((int)((this->mapUnique_->getGlobalElement(i) % (nmbPoints_oneDir * nmbPoints_oneDir)) /
                       nmbPoints_oneDir) +
                 eps) *
                h / 2;
            if ((*this->pointsUni_)[i][1] < eps && (*this->pointsUni_)[i][1] > -eps)
                (*this->pointsUni_)[i][1] = 0.0;

            (*this->pointsUni_)[i][2] =
                ((int)(this->mapUnique_->getGlobalElement(i) / (nmbPoints_oneDir * nmbPoints_oneDir) + eps)) * h / 2;
            if ((*this->pointsUni_)[i][2] < eps && (*this->pointsUni_)[i][2] > -eps)
                (*this->pointsUni_)[i][2] = 0.0;

            if ((*this->pointsUni_)[i][0] > (coorRec[0] + length - eps) ||
                (*this->pointsUni_)[i][0] < (coorRec[0] + eps) ||
                (*this->pointsUni_)[i][1] > (coorRec[1] + width - eps) ||
                (*this->pointsUni_)[i][1] < (coorRec[1] + eps) ||
                (*this->pointsUni_)[i][2] > (coorRec[2] + height - eps) ||
                (*this->pointsUni_)[i][2] < (coorRec[2] + eps)) {
                (*this->bcFlagUni_)[i] = 1;
            }
        }
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildP1_Disc_Q2_3DCube(int N, int M, int numProcsCoarseSolve,
                                                            std::string underlyingLib) {
    ...
    for (int t = 0; t < M; t++) {
        for (int s = 0; s < M; s++) {
            for (int r = 0; r < M; r++) {
                ...
                if ((*this->pointsRep_)[pCounter][0] > (coorRec[0] + length - eps) ||
                    (*this->pointsRep_)[pCounter][0] < (coorRec[0] + eps) ||
                    (*this->pointsRep_)[pCounter][1] > (coorRec[1] + width - eps) ||
                    (*this->pointsRep_)[pCounter][1] < (coorRec[1] + eps) ||
                    (*this->pointsRep_)[pCounter][2] > (coorRec[2] + height - eps) ||
                    (*this->pointsRep_)[pCounter][2] < (coorRec[2] + eps)) {
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::build3DQ1Cube(int N, int M, int numProcsCoarseSolve, std::string underlyingLib) {
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::build3DQ2Cube(int N, int M, int numProcsCoarseSolve, std::string underlyingLib) {
    ...
    for (int i = 0; i < this->mapUnique_->getNodeNumElements(); i++) {
        (*this->pointsUni_)[i][0] = (this->mapUnique_->getGlobalElement(i) % nmbPoints_oneDir) * h / 2;
        if ((*this->pointsUni_)[i][0] < eps && (*this->pointsUni_)[i][0] > -eps)
            (*this->pointsUni_)[i][0] = 0.0;

        (*this->pointsUni_)[i][1] =
            ((int)((this->mapUnique_->getGlobalElement(i) % (nmbPoints_oneDir * nmbPoints_oneDir)) / nmbPoints_oneDir) +
             eps) *
            h / 2;
        if ((*this->pointsUni_)[i][1] < eps && (*this->pointsUni_)[i][1] > -eps)
            (*this->pointsUni_)[i][1] = 0.0;
...
template <class SC, class LO, class GO, class NO>
GO MeshStructured<SC, LO, GO, NO>::globalID_Q2_20Cube(int r, int s, int t, int &rr, int off_x, int off_y, int off_z,
                                                      int M, int N, GO nmbPoints_oneDirFull, GO nmbPoints_oneDirMid) {
    ...
    if (setPoint) {
        ...
        long long sizeFullSquare =
            nmbPoints_oneDirMid * nmbPoints_oneDirFull + nmbPoints_oneDirMid * (nmbPoints_oneDirMid - 1);
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::build3DQ2_20Cube(int N, int M, int numProcsCoarseSolve,
                                                      std::string underlyingLib) {
    ...
    LO nmbPoints = ((M + 1) * (2 * (M + 1) - 1) + M * (M + 1)) * (M + 1) + ((M + 1) * (M + 1)) * M;
    ...
    vec2D_int_ptr_Type elementsVec =
        Teuchos::rcp(new std::vector<std::vector<int>>(nmbElements, std::vector<int>(20, -1)));
    ...
    for (int t = 0; t < 2 * (M + 1) - 1; t++) {
        for (int s = 0; s < 2 * (M + 1) - 1; s++) {
            rr = 0;
            for (int r = 0; r < 2 * (M + 1) - 1; r++) {
                GO index = globalID_Q2_20Cube(r, s, t, rr, offset_x, offset_y, offset_z, M, N, nmbPoints_oneDirFull,
                                              nmbPoints_oneDirMid);
    ...
    for (int t = 0; t < M; t++) {
        for (int s = 0; s < M; s++) {
            for (int r = 0; r < M; r++) {
                ...
                (*elementsVec)[counter][0] =
                    2 * (r) + (P2M + P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);
                (*elementsVec)[counter][1] =
                    2 * (r + 1) + (P2M + P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);
                (*elementsVec)[counter][2] =
                    2 * (r + 1) + (P2M + P1M) * (s + 1) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);
                (*elementsVec)[counter][3] =
                    2 * (r) + (P2M + P1M) * (s + 1) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);

                (*elementsVec)[counter][4] =
                    2 * (r) + (P2M + P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);
                (*elementsVec)[counter][5] =
                    2 * (r + 1) + (P2M + P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);
                (*elementsVec)[counter][6] =
                    2 * (r + 1) + (P2M + P1M) * (s + 1) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);
                (*elementsVec)[counter][7] =
                    2 * (r) + (P2M + P1M) * (s + 1) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);

                (*elementsVec)[counter][8] =
                    2 * (r) + 1 + (P2M + P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);
                (*elementsVec)[counter][9] =
                    (r + 1) + (P2M + P1M) * (s) + P2M + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);
                (*elementsVec)[counter][10] =
                    2 * (r) + 1 + (P2M + P1M) * (s + 1) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);
                (*elementsVec)[counter][11] =
                    r + (P2M + P1M) * (s) + P2M + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t);

                (*elementsVec)[counter][12] =
                    2 * (r) + 1 + (P2M + P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);
                (*elementsVec)[counter][13] =
                    (r + 1) + (P2M + P1M) * (s) + P2M + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);
                (*elementsVec)[counter][14] =
                    2 * (r) + 1 + (P2M + P1M) * (s + 1) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);
                (*elementsVec)[counter][15] =
                    r + (P2M + P1M) * (s) + P2M + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t + 1);

                (*elementsVec)[counter][16] =
                    (r) + (P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t) + P2M * (M + 1) + P1M * M;
                (*elementsVec)[counter][17] =
                    (r + 1) + (P1M) * (s) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t) + P2M * (M + 1) + P1M * M;
                (*elementsVec)[counter][18] = (r + 1) + (P1M) * (s + 1) +
                                              ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t) + P2M * (M + 1) + P1M * M;
                (*elementsVec)[counter][19] =
                    r + (P1M) * (s + 1) + ((P2M * (M + 1) + P1M * M) + (P1M * P1M)) * (t) + P2M * (M + 1) + P1M * M;
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::build3DQ2BFS(int N, int M, int numProcsCoarseSolve, std::string underlyingLib) {
    ...
    for (int t = 0; t < 2 * (M + 1) - 1; t++) {
        for (int s = 0; s < 2 * (M + 1) - 1; s++) {
            for (int r = 0; r < 2 * (M + 1) - 1; r++) {
                ...
                (*this->pointsRep_)[counter][0] =
                    coorRec[0] + r * h / 2. + offset_x * H + offset_Squares_x * H * nmbSubdomainsSquares_OneDir;

                (*this->pointsRep_)[counter][1] =
                    coorRec[1] + s * h / 2. + offset_y * H + offset_Squares_y * H * nmbSubdomainsSquares_OneDir;

                (*this->pointsRep_)[counter][2] =
                    coorRec[2] + t * h / 2. + offset_z * H + offset_Squares_z * H * nmbSubdomainsSquares_OneDir;

                pointsRepGlobMapping[counter] = r + s * (nmbPoints_oneDir_allSubdomain);
                ...
                } else if (offset_Squares_x > 0 && offset_Squares_z == 0 &&
                           offset_z + 1 == nmbSubdomainsSquares_OneDir && t != 2 * M) {
                    pointsRepGlobMapping[counter] -= s * (nmbPoints_oneDir - 1);
                }
                ...
                pointsRepGlobMapping[counter] +=
                    offset_x * (2 * M) + offset_y * (nmbPoints_oneDir_allSubdomain * 2 * M);
                if (offset_Squares_x > 0 && offset_Squares_z == 0 && offset_z + 1 != nmbSubdomainsSquares_OneDir) {
                ...
                pointsRepGlobMapping[counter] += offset_Squares_z * nmbPoints_oneDir_allSubdomain *
                                                 ((2 * M) * nmbSubdomainsSquares_OneDir + 1) * 2 * M *
                                                 nmbSubdomainsSquares_OneDir;
                if (offset_Squares_z > 0) {
                    pointsRepGlobMapping[counter] -=
                        (nmbPoints_oneDir - 1) * (nmbPoints_oneDir - 1) * (nmbPoints_oneDir);
                }
                counter++;
            }
        }
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildMesh2DBFS(std::string FEType, int N, int M, int numProcsCoarseSolve,
                                                    std::string underlyingLib) {
    ...
    if (FEType == "P0") {
        ...
        for (int s = 0; s < M + 1; s++) {
            for (int r = 0; r < M + 1; r++) {
                (*this->pointsRep_)[counter][0] =
                    coorRec[0] + r * h + offset_x * H + offset_Squares_x * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[counter][1] =
                    coorRec[1] + s * h + offset_y * H + offset_Squares_y * H * nmbSubdomainsSquares_OneDir;
                pointsRepGlobMapping[counter] =
                    r + s * (nmbPoints_oneDir_allSubdomain - (1 - offset_Squares_y) * (nmbPoints_oneDir - 1)) +
                    offset_x * (M) +
                ...
                if (offset_Squares_x > 0 && offset_Squares_y == 0 && offset_y + 1 == nmbSubdomainsSquares_OneDir &&
                    s == M) {
                    pointsRepGlobMapping[counter] += nmbPoints_oneDir - 1;
                }
                if ((*this->pointsRep_)[counter][0] > (coorRec[0] + length - eps) ||
                    (*this->pointsRep_)[counter][0] < (coorRec[0] + eps) ||
                    (*this->pointsRep_)[counter][1] > (coorRec[1] + height - eps) ||
                    (*this->pointsRep_)[counter][1] < (coorRec[1] + eps)) {
                    (*this->bcFlagRep_)[counter] = 1;
                }
        ...
        for (int s = 0; s < 2 * (M + 1) - 1; s++) {
            for (int r = 0; r < 2 * (M + 1) - 1; r++) {
                ...
                pointsRepGlobMapping[counter] =
                    r + s * (nmbPoints_oneDir_allSubdomain - (1 - offset_Squares_y) * (nmbPoints_oneDir - 1)) +
                    offset_x * (2 * (M + 1) - 2) +
                    offset_y * ((nmbPoints_oneDir_allSubdomain) - (1 - offset_Squares_y) * (nmbPoints_oneDir - 1)) *
                        (2 * (M + 1) - 2) +
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildMesh3DBFS(std::string FEType, int N, int M, int numProcsCoarseSolve,
                                                    std::string underlyingLib) {
        ...
        for (int t = 0; t < M + 1; t++) {
            for (int s = 0; s < M + 1; s++) {
                for (int r = 0; r < M + 1; r++) {
                    ...
                    (*this->pointsRep_)[counter][2] =
                        coorRec[2] + t * h + offset_z * H + offset_Squares_z * H * nmbSubdomainsSquares_OneDir;

                    pointsRepGlobMapping[counter] = r + s * (nmbPoints_oneDir_allSubdomain);
                    if (offset_Squares_x > 0 && offset_Squares_z == 0 && offset_z + 1 != nmbSubdomainsSquares_OneDir) {
                        pointsRepGlobMapping[counter] -= s * (nmbPoints_oneDir - 1);
                    } else if (offset_Squares_x > 0 && offset_Squares_z == 0 &&
                               offset_z + 1 == nmbSubdomainsSquares_OneDir && t != M) {
                        pointsRepGlobMapping[counter] -= s * (nmbPoints_oneDir - 1);
                    }
                    ...
                    pointsRepGlobMapping[counter] += offset_x * (M) + offset_y * (nmbPoints_oneDir_allSubdomain * M);
                    if (offset_Squares_x > 0 && offset_Squares_z == 0 && offset_z + 1 != nmbSubdomainsSquares_OneDir) {
                    ...
                    pointsRepGlobMapping[counter] += offset_Squares_z * nmbPoints_oneDir_allSubdomain *
                                                     ((M)*nmbSubdomainsSquares_OneDir + 1) * M *
                                                     nmbSubdomainsSquares_OneDir;
                    if (offset_Squares_z > 0) {
                        pointsRepGlobMapping[counter] -=
                            (nmbPoints_oneDir - 1) * (nmbPoints_oneDir - 1) * (nmbPoints_oneDir);
        ...
        for (int t = 0; t < 2 * (M + 1) - 1; t++) {
            for (int s = 0; s < 2 * (M + 1) - 1; s++) {
                for (int r = 0; r < 2 * (M + 1) - 1; r++) {
                    ...
                    pointsRepGlobMapping[counter] +=
                        offset_z * 2 * M * nmbPoints_oneDir_allSubdomain * nmbPoints_oneDir;
                    if (offset_Squares_x > 0 && offset_Squares_z == 0) {
                        pointsRepGlobMapping[counter] -= offset_z * 2 * M * (nmbPoints_oneDir - 1) * (nmbPoints_oneDir);
                    }
...
template <class SC, class LO, class GO, class NO>
void MeshStructured<SC, LO, GO, NO>::buildP1_Disc_Q2_3DBFS(int N, int M, int numProcsCoarseSolve,
                                                           std::string underlyingLib) {
    ...
    vec2D_int_ptr_Type elementsVec =
        Teuchos::rcp(new std::vector<std::vector<int>>(nmbElements, std::vector<int>(4, -1)));
    ...
    for (int t = 0; t < M; t++) {
        for (int s = 0; s < M; s++) {
            for (int r = 0; r < M; r++) {
                ...
                // point 1
                (*this->pointsRep_)[pCounter][0] =
                    coorRec[0] + r * h + offset_x * H + offset_Squares_x * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[pCounter][1] =
                    coorRec[1] + s * h + offset_y * H + offset_Squares_y * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[pCounter][2] =
                    coorRec[2] + t * h + offset_z * H + offset_Squares_z * H * nmbSubdomainsSquares_OneDir;
                ...
                // point 2
                (*this->pointsRep_)[pCounter][0] =
                    coorRec[0] + (r + 1) * h + offset_x * H + offset_Squares_x * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[pCounter][1] =
                    coorRec[1] + s * h + offset_y * H + offset_Squares_y * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[pCounter][2] =
                    coorRec[2] + t * h + offset_z * H + offset_Squares_z * H * nmbSubdomainsSquares_OneDir;
                ...
                (*this->pointsRep_)[pCounter][0] =
                    coorRec[0] + r * h + offset_x * H + offset_Squares_x * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[pCounter][1] =
                    coorRec[1] + (s + 1) * h + offset_y * H + offset_Squares_y * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[pCounter][2] =
                    coorRec[2] + t * h + offset_z * H + offset_Squares_z * H * nmbSubdomainsSquares_OneDir;
                if ((*this->pointsRep_)[pCounter][0] > (coorRec[0] + length - eps) ||
                    (*this->pointsRep_)[pCounter][0] < (coorRec[0] + eps) ||
                ...
                (*this->pointsRep_)[pCounter][1] =
                    coorRec[1] + s * h + offset_y * H + offset_Squares_y * H * nmbSubdomainsSquares_OneDir;
                (*this->pointsRep_)[pCounter][2] =
                    coorRec[2] + (t + 1) * h + offset_z * H + offset_Squares_z * H * nmbSubdomainsSquares_OneDir;
                if ((*this->pointsRep_)[pCounter][0] > (coorRec[0] + length - eps) ||
                    (*this->pointsRep_)[pCounter][0] < (coorRec[0] + eps) ||
                    (*this->pointsRep_)[pCounter][1] > (coorRec[1] + width - eps) ||
                    (*this->pointsRep_)[pCounter][1] < (coorRec[1] + eps) ||
    ...
    case 2:
        ...
        case 1: // Rectangle left inflow
            for (int i = 0; i < this->pointsUni_->size(); i++) {
                ...
                if (this->pointsUni_->at(i).at(0) > (coorRec[0] - tol) &&
                    this->pointsUni_->at(i).at(1) > (coorRec[1] + height - tol)) {
                    this->bcFlagUni_->at(i) = 1;
                }
            ...
            for (int i = 0; i < this->pointsRep_->size(); i++) {
                ...
                if (this->pointsRep_->at(i).at(0) > (coorRec[0] - tol) &&
                    this->pointsRep_->at(i).at(1) > (coorRec[1] + height - tol)) {
                    this->bcFlagRep_->at(i) = 1;
                }
        ...
        case 2: // BFS
            for (int i = 0; i < this->pointsUni_->size(); i++) {
                if (this->pointsUni_->at(i).at(0) < (coorRec[0] + tol) &&
                    this->pointsUni_->at(i).at(1) > (coorRec[1] + 1. - tol) &&
                    this->pointsUni_->at(i).at(1) < (coorRec[1] + height + tol)) {
                    this->bcFlagUni_->at(i) = 2;
                }
                if (this->pointsUni_->at(i).at(1) < (coorRec[1] + tol) ||
                    this->pointsUni_->at(i).at(1) > (coorRec[1] + height - tol) ||
                    this->pointsUni_->at(i).at(0) > (coorRec[0] + length - tol)) {
                    this->bcFlagUni_->at(i) = 1;
                }
                if (this->pointsUni_->at(i).at(0) > (coorRec[0] - tol) &&
                    this->pointsUni_->at(i).at(0) < (coorRec[0] + 1. + tol) &&
                    this->pointsUni_->at(i).at(1) > (coorRec[1] + 1. - tol) &&
                    this->pointsUni_->at(i).at(1) < (coorRec[1] + 1. + tol)) {
                    this->bcFlagUni_->at(i) = 1;
                }
            ...
            for (int i = 0; i < this->pointsRep_->size(); i++) {
                if (this->pointsRep_->at(i).at(0) < (coorRec[0] + tol) &&
                    this->pointsRep_->at(i).at(1) > (coorRec[1] + 1. - tol) &&
                    this->pointsRep_->at(i).at(1) < (coorRec[1] + height + tol)) {
                    this->bcFlagRep_->at(i) = 2;
                }
                if (this->pointsRep_->at(i).at(1) < (coorRec[1] + tol) ||
                    this->pointsRep_->at(i).at(1) > (coorRec[1] + height - tol) ||
                    this->pointsRep_->at(i).at(0) > (coorRec[0] + length - tol)) {
                    this->bcFlagRep_->at(i) = 1;
                }
                if (this->pointsRep_->at(i).at(0) > (coorRec[0] - tol) &&
                    this->pointsRep_->at(i).at(0) < (coorRec[0] + 1. + tol) &&
                    this->pointsRep_->at(i).at(1) > (coorRec[1] + 1. - tol) &&
                    this->pointsRep_->at(i).at(1) < (coorRec[1] + 1. + tol)) {
                    this->bcFlagRep_->at(i) = 1;
                }
...
void MeshStructured<SC, LO, GO, NO>::buildSurfaces(int flagsOption, string FEType) {
    ...
    switch (this->dim_) {
        ...
        //        1 * * 4 * * 0       0                         1       1 * * 4 * * 0
                    ...
                    p1[0] = this->pointsRep_->at(surfaceElements_vec[i][0]).at(0) -
                            this->pointsRep_->at(surfaceElements_vec[i][1]).at(0);
                    p1[1] = this->pointsRep_->at(surfaceElements_vec[i][0]).at(1) -
                            this->pointsRep_->at(surfaceElements_vec[i][1]).at(1);
                    p1[2] = this->pointsRep_->at(surfaceElements_vec[i][0]).at(2) -
                            this->pointsRep_->at(surfaceElements_vec[i][1]).at(2);

                    p2[0] = this->pointsRep_->at(surfaceElements_vec[i][0]).at(0) -
                            this->pointsRep_->at(surfaceElements_vec[i][2]).at(0);
                    p2[1] = this->pointsRep_->at(surfaceElements_vec[i][0]).at(1) -
                            this->pointsRep_->at(surfaceElements_vec[i][2]).at(1);
                    p2[2] = this->pointsRep_->at(surfaceElements_vec[i][0]).at(2) -
                            this->pointsRep_->at(surfaceElements_vec[i][2]).at(2);
