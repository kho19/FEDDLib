...
\tparam SC The scalar type. So far, this is always double, but having it as a template parameter would allow flexibily,
e.g., for using complex instead \tparam LO The local ordinal type. The is the index type for local indices \tparam GO
The global ordinal type. The is the index type for global indices \tparam NO The Kokkos Node type. This would allow for
performance portibility when using Kokkos. Currently, this is not used.
...
Mesh entities must include the nodes and elements. Additional information of edges and surfaces (in 3D) can be necessary
for problems where complex boundary conditions are set on surfaces or edges.

In general elements follow a hierarchical structure. For example a 3D tetrahedral element owns all 2D and 1D entities
(triangle/surface, edges) that are part of the surface.

As of now each surface and element node list is sorted to improve find/search operations. This influences for example
...
class MeshPartitioner {
  ...
  public:
    ...
    /*!
            \brief Main Function of partitioner. Called with volume ID in order to set in case it is not equal to ten.
       Always make sure the volumeID corresponds to the highest given flag.
    */
  ...
  */
    ...
    */
    void findAndSetSurfacesPartitioned(vec2D_int_Type &surfElements_vec, vec2D_int_Type &surfElements_vec_unsorted,
                                       vec_int_Type &surfElementsFlag_vec, FiniteElement &element,
                                       vec2D_int_Type &permutation, vec_GO_Type &linearSurfacePartitionOffset,
                                       int globalElID);
    ...
    */
    ...
    /*!
            \brief Only relevant in 3D. Finding the edges corresponding to the specfic element and then setting as
       subsubelement.
    */
    void findAndSetSurfaceEdges(vec2D_int_Type &edgeElements_vec, vec_int_Type &edgeElementsFlag_vec,
