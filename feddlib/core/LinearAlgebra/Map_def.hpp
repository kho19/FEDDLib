...
using namespace std;
namespace FEDD {
template <class LO, class GO, class NO> Map<LO, GO, NO>::Map() : map_() {}

template <class LO, class GO, class NO>
Map<LO, GO, NO>::Map(XpetraMapConstPtrConst_Type &xpetraMapPtrIn) : map_(xpetraMapPtrIn) {}

template <class LO, class GO, class NO> Map<LO, GO, NO>::Map(const Map_Type &mapIn) : map_() {
    ...
    map_ = Xpetra::MapFactory<LO, GO, NO>::Build(ulib, mapIn.getGlobalNumElements(), mapIn.getNodeElementList(),
                                                 mapIn.getIndexBase(), mapIn.getComm());
}

template <class LO, class GO, class NO>
Map<LO, GO, NO>::Map(std::string lib, GO numGlobalElements, const Teuchos::ArrayView<const GO> &elementList,
                     GO indexBase, const CommConstPtr_Type &comm)
    : map_() {
    Xpetra::UnderlyingLib ulib;
...
template <class LO, class GO, class NO>
Map<LO, GO, NO>::Map(std::string lib, GO numGlobalElements, LO numLocalElements, GO indexBase,
...
template <class LO, class GO, class NO> Map<LO, GO, NO>::~Map() {}

template <class LO, class GO, class NO> std::string Map<LO, GO, NO>::getUnderlyingLib() const {
...
typename Map<LO, GO, NO>::MapPtr_Type Map<LO, GO, NO>::buildVecFieldMap(UN numDofs, std::string ordering) const {
    ...
    return Teuchos::rcp(new Map_Type(this->getUnderlyingLib(), GOOT::invalid(), elementListField(),
                                     map_->getIndexBase(), map_->getComm()));
}
...
template <class LO, class GO, class NO> GO Map<LO, GO, NO>::getGlobalElement(LO id) const {
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(), std::runtime_error, "map is null.");
    return map_->getGlobalElement(id);
}

template <class LO, class GO, class NO> LO Map<LO, GO, NO>::getLocalElement(GO id) const {
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(), std::runtime_error, "map is null.");
    return map_->getLocalElement(id);
}

template <class LO, class GO, class NO> LO Map<LO, GO, NO>::getNodeNumElements() const {
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(), std::runtime_error, "map is null.");
    return map_->getLocalNumElements();
}

template <class LO, class GO, class NO> GO Map<LO, GO, NO>::getGlobalNumElements() const {
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(), std::runtime_error, "map is null.");
    return map_->getGlobalNumElements();
}

template <class LO, class GO, class NO> Teuchos::ArrayView<const GO> Map<LO, GO, NO>::getNodeElementList() const {
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(), std::runtime_error, "map is null.");
    return map_->getLocalElementList();
}

template <class LO, class GO, class NO> GO Map<LO, GO, NO>::getMaxAllGlobalIndex() const {
    return map_->getMaxAllGlobalIndex();
}

template <class LO, class GO, class NO> LO Map<LO, GO, NO>::getMaxLocalIndex() const {
    return map_->getMaxLocalIndex();
}

template <class LO, class GO, class NO> void Map<LO, GO, NO>::print(Teuchos::EVerbosityLevel verbLevel) const {

    Teuchos::RCP<Teuchos::FancyOStream> out = Teuchos::VerboseObjectBase::getDefaultOStream();
    map_->describe(*out, verbLevel);
}

template <class LO, class GO, class NO> typename Map<LO, GO, NO>::CommConstPtr_Type Map<LO, GO, NO>::getComm() const {

    return map_->getComm();
}

template <class LO, class GO, class NO> typename Map<LO, GO, NO>::CommPtr_Type Map<LO, GO, NO>::getCommNonConst() {
...
Teuchos::RCP<Map<LO, GO, NO>> Map<LO, GO, NO>::buildUniqueMap(int numFreeProcs) const {
    ...
    if (numFreeProcs == 0) {
        ...
        // Build corresponding unique map
        Teuchos::RCP<Xpetra::Map<LO, GO, NO>> mapXpetra =
            Xpetra::MapFactory<LO, GO, NO>::Build(map_->lib(), -1, uniqueVector(), 0, map_->getComm());
        ...
        Teuchos::RCP<Xpetra::Map<LO, GO, NO>> linearMapAvailRanks = Xpetra::MapFactory<LO, GO, NO>::Build(
            map_->lib(), Teuchos::OrdinalTraits<GO>::invalid(), myElements(), 0, map_->getComm());

        Teuchos::RCP<Xpetra::Vector<GO, LO, GO, NO>> globalIndices =
            Xpetra::VectorFactory<GO, LO, GO, NO>::Build(linearMapAvailRanks);

        Teuchos::RCP<Xpetra::Import<LO, GO, NO>> importer =
            Xpetra::ImportFactory<LO, GO, NO>::Build(map_, linearMapAvailRanks);
        Teuchos::RCP<Xpetra::Import<LO, GO, NO>> importer2 =
            Xpetra::ImportFactory<LO, GO, NO>::Build(linearMapAvailRanks, map_);
...
template <class LO, class GO, class NO> GO Map<LO, GO, NO>::getIndexBase() const {
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(), std::runtime_error, "map is null.");
