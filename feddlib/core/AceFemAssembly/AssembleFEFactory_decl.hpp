...
\tparam SC The scalar type. So far, this is always double, but having it as a
template parameter would allow flexibily, e.g., for using complex instead
...
(this should actually by removed since the class should operate only on element
level) \tparam NO The Kokkos Node type. This would allow for performance
portibility when using Kokkos. Currently, this is not used.

This class provides a function for constructing objects of classes derived from
AssembleFE. In particular, an AssembleFE object is constructed as follows: \code
AssembleFE assmeblyFe =
AssembleFEFactory<>::build("problemType",flag,nodesRefConfig,params); \endcode
If a new element assembly is implemented, it has to be added to the build()
function. Only then, this element assembly can be executed within the FEDDLib.
*/
template <class SC, class LO, class GO, class NO> class AssembleFEFactory {
public:
  ...
  */
  ...
  /*!
   \brief We only need this one function to build assembleFE, where we define
   the problem type and the AssembleFE object we extend to a specific problem.
  ...
  */
  AssembleFEPtr_Type build(string problemType, int flag,
