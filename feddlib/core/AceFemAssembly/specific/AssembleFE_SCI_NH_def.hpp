...
AssembleFE_SCI_NH<SC, LO, GO, NO>::AssembleFE_SCI_NH(
  ...
  poissonRatio_ =
      this->params_->sublist("Parameter Solid").get("Poisson Ratio", 0.49e-0);
...
void AssembleFE_SCI_NH<SC, LO, GO, NO>::assembleJacobian() {
  ...
  SmallMatrixPtr_Type elementMatrix =
      Teuchos::rcp(new SmallMatrix_Type(dofsElement_, 0.));
...
template <class SC, class LO, class GO, class NO>
void AssembleFE_SCI_NH<SC, LO, GO, NO>::assembleDeformationDiffusionNeoHook(
    SmallMatrixPtr_Type &elementMatrix) {
  ...
  AceGenInterface::DeformationDiffusionNeoHookTetrahedra3D10 neoHookeElement(
      &positions[0], &displacements[0], &concentrations[0],
      &concentrationsLastConverged[0], &domainData[0], timeIncrement,
      integrationCode);
  neoHookeElement.computeTangentResidual();
