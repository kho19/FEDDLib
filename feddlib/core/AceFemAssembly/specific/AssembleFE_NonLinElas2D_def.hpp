...
AssembleFE_NonLinElas2D<SC, LO, GO, NO>::AssembleFE_NonLinElas2D(int flag, vec2D_dbl_Type nodesRefConfig,
    ...
    E_ = this->params_->sublist("Parameter")
             .get("E", 1000.0); // the last value is the dafault value, in case no parameter is set
    ...
    /// string: Physical Entity (i.e. Velocity) , string: Discretisation (i.e. "P2"), int: Degrees of Freedom per Node,
    /// int: Number of Nodes per element)
...
template <class SC, class LO, class GO, class NO> void AssembleFE_NonLinElas2D<SC, LO, GO, NO>::assembleJacobian() {
    ...
    SmallMatrixPtr_Type elementMatrix =
        Teuchos::rcp(new SmallMatrix_Type(dofsElement_)); // Matrix we fill with entries.
...
*/
template <class SC, class LO, class GO, class NO>
void AssembleFE_NonLinElas2D<SC, LO, GO, NO>::assemblyNonLinElas(SmallMatrixPtr_Type &elementMatrix) {

    // TODO: kho put the 2D interface here
    ...
    skr3(&v[0], &d[0], &ul[0], &ul0[0], &xl[0], &s[0], &p[0], &ht[0],
         &hp[0]); // Fortran subroutine call modifies s and p
...
template <class SC, class LO, class GO, class NO> void AssembleFE_NonLinElas2D<SC, LO, GO, NO>::assembleRHS() {
    ...
    // [Efficiency] Need to know which is called first: assembleRHS() or assembleJacobian(), so that multiple calls to
    // skr() may be avoided. Note skr() computes both elementMatrix_ and rhsVec_
    // TODO: kho replace 2D interface here
    std::vector<double> v(1066); // Working vector, size defined by AceGen-FEAP
    std::vector<double> d(2);    // Material parameters
    std::vector<double> ul(30);  // The solution vector(or displacement in this case)
    std::vector<double> ul0(30); // Currently unused but must be passed to match FEAP template
...
template <class SC, class LO, class GO, class NO>
void AssembleFE_NonLinElas2D<SC, LO, GO, NO>::updateParameter(string type, double value) {
    if (type == "E")
        this->E_ = value;
}
