...
/*!
 ...
 \brief Assembly Jacobian is simply assemblyLaplacian for nonlinear Laplace
Problem
...
void AssembleFENonLinLaplace<SC, LO, GO, NO>::assembleJacobian() {
  ...
  // TODO implement the variational form that you derived here
...
*/
template <class SC, class LO, class GO, class NO>
void AssembleFENonLinLaplace<SC, LO, GO, NO>::assemblyLaplacian(
    SmallMatrixPtr_Type &elementMatrix) {

  int dim = this->getDim();
  int numNodes =
      std::get<3>(this->diskTuple_->at(0)); // this->getNodesRefConfig().size();
  ...
  // TODO multiply by the nonlinear factor in here
...
void AssembleFENonLinLaplace<SC, LO, GO, NO>::assembleRHS() {
  // TODO the residual for the nonlinear laplacian should be exactly the
  // laplacian above.
  int dim = this->getDim();
