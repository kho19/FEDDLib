...
typename AssembleFEFactory<SC,LO,GO,NO>::AssembleFEPtr_Type AssembleFEFactory<SC,LO,GO,NO>::build(string problemType, int flag, vec2D_dbl_Type nodesRefConfig, ParameterListPtr_Type params,tuple_disk_vec_ptr_Type tuple)
{
    ...
    Teuchos::RCP<AssembleFENonLinLaplace<SC, LO, GO, NO>> assembleFESpecific(new AssembleFENonLinLaplace<SC, LO, GO, NO>(flag, nodesRefConfig, params, tuple));
    assembleFE = assembleFESpecific;
  }
	...
	else if(problemType == "NonLinearElasticity2D"){
		Teuchos::RCP<AssembleFE_NonLinElas2D<SC,LO,GO,NO>> assembleFESpecific(new AssembleFE_NonLinElas2D<SC,LO,GO,NO>(flag,nodesRefConfig, params,tuple) );
		assembleFE = assembleFESpecific;
	}
